...
public class RecipeGUI {
    ...
    public void openRecipeCreator(Player player, String type) {
        ...
        switch (type) {
            case "furnace" -> {
                creatorFurnaceTime.put(uuid, 10); // 熔炉10s，高炉自动=一半
                creatorFurnaceMode.put(uuid, "both"); // 默认熔炉+高炉
            }
    ...
    public void adjustCreatorValue(Player player, String slotChar, boolean increase) {
        ...
        if ("P".equals(slotChar)) {
            // 烧炼模式: blast → 调高炉时间, furnace/both → 调熔炉时间
    ...
    /** 构建模式切换按钮：blast→高炉材质，furnace→熔炉材质，both→yml配置材质 */
    private ItemStack buildModeButton(ButtonDef btn, String mode) {
        Material mat = switch (mode) {
            ...
            default -> btn.material(); // both: 使用 yml 配置的 GREEN_STAINED_GLASS_PANE
        };
        ItemStack item = new ItemStack(mat != null ? mat : Material.PAPER);
        ItemMeta meta = item.getItemMeta();
        if (meta != null) {
            String name = btn.name().replace("{mode}", furnaceModeDisplay(mode));
        ...
        switch (type) {
            ...
            case "furnace" -> {
                ...
                if (!"furnace".equals(mode)) {
                    int bTime;
                    if ("blast".equals(mode))
                        bTime = creatorTime.getOrDefault(uuid, 5); // 仅高炉模式直接调高炉时间
                    else
                        bTime = Math.max(1, creatorFurnaceTime.getOrDefault(uuid, 10) / 2); // both 自动=熔炉÷2
    ...
    private List<CEBridge.RecipeData> filterRecipes(List<CEBridge.RecipeData> all, String query) {
        ...
        return all.stream().filter(r -> {
            ...
            for (String ing : r.allIngredientIds) { if (matchItem(ing, q)) return true; }
