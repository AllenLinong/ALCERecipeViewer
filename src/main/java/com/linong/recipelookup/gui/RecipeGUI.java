...
public class RecipeGUI {
    ...
    public void openRecipeList(Player player, String categoryId, int page) {
        ...
        List<CEBridge.RecipeData> recipes = getSortedRecipes(categoryId, filter, locale, player.getUniqueId());
    ...
    private List<CEBridge.RecipeData> filterRecipes(List<CEBridge.RecipeData> all, String query, Locale locale, UUID uuid) {
        String q = query.toLowerCase();
        String mode = searchMode.getOrDefault(uuid, SEARCH_MODE_RESULT);
        return all.stream()
                .filter(r -> {
                    if (SEARCH_MODE_INGREDIENT.equals(mode)) {
                        return matchIngredients(r, q, locale);
                    }
                    return matchItem(r.resultId, q, locale);
                })
                .collect(Collectors.toList());
    }

    private boolean matchIngredients(CEBridge.RecipeData r, String query, Locale locale) {
        for (String ingId : r.allIngredientIds) {
            if (matchItem(ingId, query, locale)) return true;
        }
        return false;
    }
    ...
    public String getSearchMode(UUID uuid) { return searchMode.getOrDefault(uuid, SEARCH_MODE_RESULT); }
    public void toggleSearchMode(Player player) {
        UUID uuid = player.getUniqueId();
        String current = searchMode.getOrDefault(uuid, SEARCH_MODE_RESULT);
        String next = SEARCH_MODE_RESULT.equals(current) ? SEARCH_MODE_INGREDIENT : SEARCH_MODE_RESULT;
        searchMode.put(uuid, next);
        // 刷新列表以更新按钮显示模式
        String category = playerCategory.getOrDefault(uuid, "crafting");
